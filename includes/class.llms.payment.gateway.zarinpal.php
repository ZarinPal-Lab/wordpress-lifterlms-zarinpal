<?php
/**
 * Zarinpal Payment Gateway for LifterLMS
 * @since    1.0.0
 * @version  1.0.0
 * @author   Zarinpal
 */
...
class LLMS_Payment_Gateway_Zarinpal extends LLMS_Payment_Gateway {




    const REDIRECT_URL = 'https://www.zarinpal.com/pg/StartPay/';
    ...
    public function __construct() {
        ...
        $this->id = 'Zarinpal';
        $this->icon = '<a href="https://www.zarinpal.com"  onclick="javascript:window.open(\'https://www.zarinpal.com\',\'zarinpal\',\'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=1060, height=700\'); return false;"><img src="https://cdn.zarinpal.com/badges/trustLogo/1.png" border="0" alt="zarinpal logo"></a>';
        $this->admin_description = __( 'Allow customers to purchase courses and memberships using Zarinpal.', 'lifterlms-Zarinpal' );
        $this->admin_title = 'Zarinpal';
        $this->title = 'Zarinpal';
        $this->description = __( 'Pay via Zarinpal', 'lifterlms-Zarinpal' );
        ...
        // add Zarinpal specific fields
        add_filter( 'llms_get_gateway_settings_fields', array( $this, 'settings_fields' ), 10, 2 );

        // output Zarinpal account details on confirm screen
    ...
    public function after_payment_method_details() {
        ...
        if ( ! $order || 'Zarinpal' !== $order->get( 'payment_gateway' ) ) {
            return;
        }

        echo '<input name="llms_zarinpal_token" type="hidden" value="' . $_GET['Authority'] . '">';
    ...
    public function confirm_pending_order( $order ) {
        ...
        $this->log( 'Zarinpal `after_payment_method_callback()` started', $order, $_POST );
    ...
    public function handle_pending_order( $order, $plan, $person, $coupon = false ) {
        ...
        $this->log( 'Zarinpal `handle_pending_order()` started', $order, $plan, $person, $coupon );
...
<script type="text/javascript">
    ...
    public function settings_fields( $fields, $gateway_id ) {
        ...
        $fields[] = array(
            'type'  => 'custom-html',
            'value' => '
				<h4>' . __( 'Zarinpal Settings', 'lifterlms-Zarinpal' ) . '</h4>
				<p>' . __( 'مرچنت کد زرین پال خود را برای اتصال به زرین پال وارد کنید ', 'lifterlms-Zarinpal' ) . '</p>
			',
        );

        $settings = array(
            'MerchantID' => __( 'مرچنت کد', 'lifterlms-Zarinpal' ),
        );
